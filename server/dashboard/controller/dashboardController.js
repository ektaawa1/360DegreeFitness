...
exports.nutrition_info = async (req, res) => {
    ...
    try {
        ...
        const response = await axios.get(`${FASTAPI_BASE_URL}/getWeeklyNutritionSummary?user_id=${user_id}`);
...
exports.exercise_info = async (req, res) => {
    ...
    try {
        const user_id = verified.id;
        const response = await axios.get(`${FASTAPI_BASE_URL}/getWeeklyExerciseSummary?user_id=${user_id}`);
        return res.json(response.data);
